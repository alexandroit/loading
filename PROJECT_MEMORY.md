# @stackline/loading project memory

Last updated: 2026-05-22

## Current release

- Package: `@stackline/loading`
- Stable version: `1.0.0`
- Public npm registry: blocked by npm registry state
...
- Published `@stackline/loading@1.0.0` to the local Verdaccio registry.
- Updated the local test app to install `@stackline/loading@1.0.0` from Verdaccio.
...
Publishing to `https://registry.npmjs.org/` was attempted for `@stackline/loading@1.0.0`, but npm rejected the package with `E409`.
...
Because of that registry state, the public catalog currently avoids showing a broken npm button for this package.
...
npm run build:all
npm pack --dry-run
npm view @stackline/loading versions --registry http://192.168.3.52:4873/
```

Expected package check from the local test app:

```text
@stackline/loading@1.0.0
```
...
```text
/storage/data/github/tests/stackline-loading-test
```

The app installs `@stackline/loading@1.0.0` from local Verdaccio and runs with Vite.

Useful commands:

```bash
cd /storage/data/github/tests/stackline-loading-test
npm ls @stackline/loading --depth=0
npm run build
