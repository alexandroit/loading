...
loader.show();`}function be(e){return{variant:e.variant,size:e.size,speed:e.speed,color:e.color,secondaryColor:e.secondaryColor,overlay:e.overlay||e.fullscreen,centered:e.centered||e.overlay||e.fullscreen,fullscreen:!1,label:e.label,delay:e.delay,minVisible:e.minVisible}}function fe(e,r){const a=r?"is-fullscreen-simulated":"";return e==="table"?`
  ...
  `}function ye(e){let r=null;const a={...f};let t="default";e.innerHTML=`
    <div class="playground-shell">
      <form class="playground-controls">
        <div class="field">
          <label for="variant">Variant</label>
          <select id="variant" name="variant">
            ${N.map(d=>`<option value="${d.value}">${d.label}</option>`).join("")}
          </select>
        </div>
        <div class="field">
  ...
  `;const i=e.querySelector(".playground-controls"),l=e.querySelector("[data-playground-host]"),n=e.querySelector("[data-playground-code]"),p=e.querySelector("[data-size-value]"),u=e.querySelector("[data-speed-value]"),x=e.querySelector("[data-replay]"),v=e.querySelector("[data-copy-playground]"),z=e.querySelector(".playground-stage"),A=Array.from(e.querySelectorAll("[data-preview-tab]"));if(!i||!l||!n||!p||!u||!x||!v||!z)return()=>{};const C=()=>{r==null||r.destroy(),r=null,l.innerHTML=fe(t,a.fullscreen),z.classList.toggle("is-reduced-motion",a.reducedMotion),z.dataset.previewTab=t;const d=l.querySelector("[data-surface]")??l;r=L({...be(a),target:d}),r.mount(d),r.show(),n.textContent=ge(a),p.textContent=`${a.size}px`,u.textContent=`${a.speed}ms`},G=()=>{const d=new FormData(i);a.variant=d.get("variant")??f.variant,a.size=Number(d.get("size")??f.size),a.speed=Number(d.get("speed")??f.speed),a.color=String(d.get("color")??f.color),a.secondaryColor=a.color==="#2563eb"?f.secondaryColor:`color-mix(in srgb, ${a.color} 20%, transparent)`,a.label=String(d.get("label")??""),a.delay=Number(d.get("delay")??0),a.minVisible=Number(d.get("minVisible")??f.minVisible),a.overlay=d.get("overlay")==="on",a.fullscreen=d.get("fullscreen")==="on",a.centered=d.get("centered")==="on",a.reducedMotion=d.get("reducedMotion")==="on",C()};return i.addEventListener("input",G),x.addEventListener("click",()=>{r&&r.hide().then(()=>r==null?void 0:r.show())}),v.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(n.textContent??""),v.textContent="Copied",window.setTimeout(()=>{v.textContent="Copy code"},1200)}catch{v.textContent="Copy failed"}}),A.forEach(d=>{d.addEventListener("click",()=>{const $=d.dataset.previewTab;!$||$===t||(t=$,A.forEach(S=>{const U=S===d;S.classList.toggle("is-active",U),S.setAttribute("aria-selected",U?"true":"false")}),C())})}),C(),()=>{r==null||r.destroy()}}const we=["galaxy","astronaut-to-mars","baseball-player","football-player"],g={install:"npm install @revivejs/loading",quickStart:`import { createLoader } from '@revivejs/loading';
...
fetch('/api/revenue')
  .finally(() => loader.hide())
  .finally(() => loader.destroy());`,theme:`import { applyThemeTokens, darkTheme } from '@revivejs/loading';
...
loader.show();`},D=[{id:"dashboard-card",title:"Dashboard card",description:"A polished card overlay keeps the layout stable while a metric tile refreshes.",variant:"orbit",options:{variant:"orbit",size:44,overlay:!0,centered:!0,label:"Refreshing KPI"},previewHtml:`
...
});`}];function xe(e){return e.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;").split('"').join("&quot;").split("'").join("&#39;")}function m(e,r,a="ts"){return`
    <div class="code-block">
      <div class="code-block-head">
        <span>${e}</span>
        <button class="copy-button" type="button">Copy</button>
      </div>
      <pre><code class="language-${a}">${xe(r)}</code></pre>
    </div>
...
`,T=[{slug:"home",title:"@revivejs/loading",eyebrow:"Premium Loading Toolkit",description:"A modern framework-agnostic loading library for overlays, buttons, cards, dashboards, and polished async UX.",body:`
      <section class="hero">
        <div class="hero-copy">
  ...
  `}function Y(){if(!O)return;w.forEach(r=>r()),w.clear();const e=$e(Ce());O.innerHTML=`
  ...
  `,window.scrollTo({top:0,behavior:"instant"}),F(),Ue()}function F(){var e;document.querySelectorAll("[data-theme-toggle]").forEach(r=>{r.addEventListener("click",Ee)}),(e=document.querySelector("[data-nav-toggle]"))==null||e.addEventListener("click",()=>{document.body.classList.toggle("is-nav-open")}),document.querySelectorAll(".copy-button").forEach(r=>{r.addEventListener("click",async()=>{var t,i;const a=((i=(t=r.closest(".code-block"))==null?void 0:t.querySelector("code"))==null?void 0:i.textContent)??"";try{await navigator.clipboard.writeText(a),r.textContent="Copied",window.setTimeout(()=>{r.textContent="Copy"},1200)}catch{r.textContent="Copy failed"}})}),document.querySelectorAll(".nav-link").forEach(r=>{r.addEventListener("click",()=>{document.body.classList.remove("is-nav-open")})})}function Le(e,r){e.innerHTML=r.map(a=>{const t=k(a);return`
      ...
      `}).join(""),e.querySelectorAll("[data-variant-host]").forEach(a=>{const t=a.dataset.variantHost,i={variant:t,size:t==="shimmer"?56:44,centered:!0,label:"",target:a,visible:!0},l=t.includes("dots")?4:t.includes("bars")?5:null,n=L(l==null?i:{...i,count:l});n.mount(a),n.show(),w.add(()=>n.destroy())})}function Ae(e){e.innerHTML=D.map(r=>`
      ...
      `).join(""),e.querySelectorAll("[data-example-id]").forEach(r=>{const a=r.dataset.exampleId,t=D.find(n=>n.id===a);if(!t)return;const i=r.querySelector(t.targetSelector)??r,l=L({...t.previewOptions??t.options,target:i});l.mount(i),l.show(),w.add(()=>l.destroy())}),F()}function Ue(){document.querySelectorAll("[data-gallery]").forEach(e=>{const a=e.dataset.gallery==="featured"?we:N.map(t=>t.value);Le(e,a)}),document.querySelectorAll("[data-example-grid]").forEach(e=>{Ae(e)}),document.querySelectorAll("[data-playground]").forEach(e=>{const r=ye(e);w.add(r)})}function X(e){document.documentElement.dataset.docsTheme=e,me(document.documentElement,e==="light"?ue:he),window.localStorage.setItem("revivejs-loading-docs-theme",e)}function Ee(){const e=document.documentElement.dataset.docsTheme==="light"?"dark":"light";X(e)}function Pe(){const e=window.localStorage.getItem("revivejs-loading-docs-theme");X(e==="light"?"light":"dark")}Pe();window.addEventListener("hashchange",Y);Y();
//# sourceMappingURL=index-B6C-pIID.js.map
